...
class Conduit
{
	...
	{
		if (!is_string($public)) {
			throw new InvalidArgumentException('Expecting $public to be type string');
		}
		if (!is_string($private)) {
			throw new InvalidArgumentException('Expecting $private to be type string');
		}
		if (!is_string($url)) {
			throw new InvalidArgumentException('Expecting $url to be type string');
		}
		if (!filter_var($url, FILTER_VALIDATE_URL)) {
			throw new InvalidArgumentException('Expecting $url to pass FILTER_VALIDATE_URL filter');
		}
	...
	public function token ($payload, $url)
	{
		...
		if (!is_string($payload)) {
			throw new InvalidArgumentException('Expecting $payload to be type string');
		}
	...
	{
		if (!is_string($hash)) {
			throw new InvalidArgumentException('Expecting $hash to be type string');
		}
	...
	{
		if (!is_string($path)) {
			throw new InvalidArgumentException('Expecting $path to be type string');
		}
